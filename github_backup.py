...
환경변수 (Render):
  ...
  GITHUB_PATH   : 'news_track_log.jsonl'         (레포 내 경로)
  GITHUB_BRANCH : 'main'                          (기본값)

사용 (news_smart_v5.py 에서):
  import github_backup
  github_backup.restore_from_github(LOG_PATH)   # 시작 시 1번 (복원)
  ...발송 후...
  github_backup.backup_to_github(LOG_PATH)      # log_sent 직후
"""
...
def backup_to_github(local_path, msg=None):
    """로컬 jsonl 전체를 깃허브에 커밋(PUT). 성공 시 True."""
    if not TOKEN:
    ...
    try:
        ...
        r = requests.put(API, headers=_headers(), json=payload, timeout=20)
        if r.status_code in (200, 201):
            print(f"  [github_backup] 깃허브 커밋 성공 ({len(content)} bytes)", flush=True)
            return True
        else:
...
def restore_from_github(local_path):
    """깃허브에서 jsonl 다운로드 → 로컬에 복원. 시작 시 1번 호출.
    ...
    try:
        r = requests.get(API, headers=_headers(),
                         params={'ref': BRANCH}, timeout=15)
        if r.status_code != 200:
            print(f"  [github_backup] 복원할 파일 없음 (status {r.status_code})", flush=True)
            return False
        data = r.json()
        ...
        if os.path.exists(local_path):
            ...
            if local_size >= len(content):
                print(f"  [github_backup] 로컬({local_size})이 최신 — 복원 생략", flush=True)
                return False
        with open(local_path, 'wb') as f:
            f.write(content)
        n_lines = content.decode('utf-8', 'ignore').count('\n')
        print(f"  [github_backup] 깃허브에서 복원 ({len(content)} bytes, {n_lines}줄)", flush=True)
        return True
    except Exception as e:
